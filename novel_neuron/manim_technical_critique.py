...
A long-form video explaining the journey from original ONC claims to corrected
oscillator reservoir computing framework.

Run: manim -pqh manim_technical_critique.py TechnicalCritiqueVideo
"""
...
class TechnicalCritiqueVideo(Scene):
    ...
    def part1_introduction(self):
        ...
        claims = VGroup(
            ...
            Text("• 10¹² ferroelectric endurance", font_size=32),
            Text("• Universal Adler equation applicability", font_size=32),
            Text("• 8× higher information density", font_size=32),
    ...
    def part2_original_claims(self):
        ...
        comparison = VGroup(
            ...
            Text("O(1) summation → Lock time ∝ N^α → FATAL", font_size=20),
            Text("10¹² endurance → 10⁴-10⁹ cycles → HIGH", font_size=20),
            Text("Universal Adler → Fails for ring → HIGH", font_size=20),
            Text("8× information density → Ignores time → HIGH", font_size=20),
        ...
        explanation = VGroup(
            Text("Original: τ_lock ≈ constant (coupling scales with N)", font_size=28),
            Text("Reality: τ_lock ∝ N^α, α ∈ [0.5, 1.0]", font_size=28),
    ...
    def part3_literature_survey(self):
        ...
        papers = VGroup(
            ...
            Text("   HfO₂ ferroelectric capacitors", font_size=24),
            Text("   Endurance: 10⁴-10⁹ cycles (NOT 10¹²)", font_size=24, color=RED)
        ).arrange(DOWN).next_to(title, DOWN, buff=1)
    ...
    def part4_mathematical_corrections(self):
        ...
        eq1 = MathTex(r"MC \leq N \cdot \frac{K}{K + \gamma}").scale(1.5).next_to(theorem1, DOWN, buff=1)
        self.play(Write(eq1))
        self.wait(1)
        ...
        explanation1 = VGroup(
            ...
            Text("γ: Phase noise strength", font_size=24),
            Text("→ Phase noise fundamentally limits memory", font_size=24, color=YELLOW)
        ).arrange(DOWN, aligned_left).next_to(eq1, DOWN, buff=1)
        ...
        eq4 = MathTex(r"E_{op} \geq \frac{N P_{osc}}{f_{osc}}").scale(1.5).next_to(theorem4, DOWN, buff=1)
        self.play(Write(eq4))
        self.wait(1)
        ...
        ).arrange(DOWN, aligned_left).next_to(eq4, DOWN, buff=1)
        ...
        eq_obs = MathTex(r"\tau \geq \frac{2^N}{f_{osc}}").scale(1.5).next_to(obs_time, DOWN, buff=1)
        self.play(Write(eq_obs))
        self.wait(1)
        
        explanation_obs = VGroup(
            Text("For N-bit precision, need τ ≥ 2^N / f_osc", font_size=24),
            Text("Example: 8-bit precision at 1 GHz → τ ≥ 256 ns", font_size=24),
            Text("→ High precision fundamentally limits throughput", font_size=24, color=YELLOW),
            Text("→ Original '8× information density' ignored this", font_size=24, color=RED)
        ).arrange(DOWN, aligned_left).next_to(eq_obs, DOWN, buff=1)
    ...
    def part5_simulation_results(self):
        ...
        ).arrange(DOWN, aligned_left).next_to(mc_title, DOWN, buff=1)
        ...
        ).arrange(DOWN, aligned_left).next_to(scale_title, DOWN, buff=1)
    ...
    def part6_corrected_framework(self):
        ...
        kuramoto = MathTex(
            r"\frac{d\phi_i}{dt} = \omega_i + \sum_{j=1}^N K_{ij} \sin(\phi_j - \phi_i) + \xi_i(t)"
        ).scale(1.2).next_to(math_title, DOWN, buff=1)
        self.play(Write(kuramoto))
        self.wait(1)
        
        readout = MathTex(
            r"\mathbf{y}(t) = \mathbf{W}_{out} \cdot \mathbf{R}(\mathbf{x}(t))"
        ).scale(1.2).next_to(kuramoto, DOWN, buff=1)
        ...
        theorem_list = VGroup(
            Text("1. Memory Capacity Bound: MC ≤ N·K/(K+γ)", font_size=24),
            Text("2. Universal Approximation: Can approximate any continuous function", font_size=24),
            Text("3. Channel Capacity Bound: C ≤ N·∫log(1+SNR)df", font_size=24),
            Text("4. Energy-Efficiency Bound: E_op ≥ N·P_osc/f_osc", font_size=24),
            Text("5. Synchronization Time: τ_sync ∝ 1/λ₂(L)", font_size=24),
            Text("6. Memory vs Topology: MC ∝ ρ(A)·K/(K+γ)", font_size=24),
            Text("7. Noise Robustness: SNR = K²σ²_input/(S_ξ·Tr[L⁻¹])", font_size=24)
        ).arrange(DOWN, aligned_left).next_to(theorems_title, DOWN, buff=1)
    ...
    def part7_true_use_case(self):
        ...
        advantages = VGroup(
            ...
            Text("6× lower energy per sample", font_size=22),
        ...
        optimal_specs = VGroup(
            ...
            Text("Energy per detection: 25 μJ vs 80 μJ (3.2×)", font_size=22, color=GREEN),
            Text("Latency: 50 ms vs 100 ms (2×)", font_size=22, color=GREEN),
            Text("Battery life: 200h vs 125h (1.6×)", font_size=22, color=GREEN)
        ...
        not_advantages = VGroup(
            Text("1. High-Precision Inference (10-100× disadvantage)", font_size=28),
            Text("2. High-Throughput Computing (10⁶× disadvantage)", font_size=28),
            Text("3. Training/Learning (10-100× disadvantage)", font_size=28),
            Text("4. General-Purpose GPU Replacement (fundamentally wrong paradigm)", font_size=28)
        ).arrange(DOWN, aligned_left).next_to(not_title, DOWN, buff=1)
    ...
    def part8_research_roadmap(self):
        ...
        mve_specs = VGroup(
            ...
            Text("Exceptional: >92% accuracy, <500 μW, <50 ms", font_size=20)
        ...
        gogo_criteria = VGroup(
            ...
            Text("Energy advantage >2× vs digital", font_size=20),
            Text("Cost advantage >1.5× vs digital", font_size=20),
    ...
    def part9_conclusion(self):
        ...
        summary = VGroup(
            ...
            Text("10¹² endurance: FALSE (literature: 10⁴-10⁹)", font_size=22),
            ...
            Text("Lock time scales as O(N^α), α ∈ [0.5, 1.0]", font_size=22),
